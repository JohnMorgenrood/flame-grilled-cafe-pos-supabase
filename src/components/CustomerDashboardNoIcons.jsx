...
const CustomerDashboardNoIcons = () => {
  const navigate = useNavigate();
  const [activeTab, setActiveTab] = useState('overview');

  // Modern mock data with more details
  const customer = {
    name: 'Sarah Johnson',
  ...
  const activeOrders = orders.filter(order => order.status === 'preparing' || order.status === 'confirmed');
  ...
  const StatCard = ({ icon, title, value, subtitle, color = 'text-blue-600' }) => (
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center space-x-3">
              <span className={`px-3 py-1 text-xs font-medium rounded-full border ${getStatusColor(order.status)}`}>
                {order.status === 'preparing' && <span className="inline mr-1">⏰</span>}
                {order.status === 'completed' && <span className="inline mr-1">✅</span>}
          ...
          <div className="border-b border-gray-100">
            <nav className="flex space-x-8 px-6">
              {[
                ...
                { id: 'active', label: `Active Orders (${activeOrders.length})`, icon: '⏰' },
...
export default CustomerDashboardNoIcons;
